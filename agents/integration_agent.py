...
class IntegrationAgent(BaseAgent, ResearchAwareMixin):
    ...
    def process_task(self, task: Task) -> Task:
        ...
        try:
            ...
            # Extract API information from research
            api_info = {}
            if research_results:
                api_info = self.extract_api_info_from_research(research_results)
                task.metadata['research_context'] = api_info
                self.logger.info(f"Enriched task with research: {len(api_info.get('documentation_urls', []))} docs, "
            ...
            # Generate integration code (passing research context)
            files_created = []
            
            if "quickbooks" in description or "qbo" in description:
                files_created.extend(self._create_quickbooks_integration(task, api_info))
            
            if "odoo" in description:
                files_created.extend(self._create_odoo_integration(task, api_info))
            
            if "stripe" in description or "billing" in description:
                files_created.extend(self._create_stripe_integration(task, api_info))
    ...
    def _create_quickbooks_integration(self, task: Task, api_info: Dict = None) -> List[str]:
        """Create QuickBooks integration code using research context."""
        files_created = []
        description = task.description.lower()
...
'''
    ...
    def _create_qbo_client(self, task: Task) -> str:
        """Create QuickBooks API client with FULL entity support."""
        file_path = os.path.join(self.project_layout.api_clients_dir, "qbo.py")
        full_path = os.path.join(self.workspace_path, file_path)
        os.makedirs(os.path.dirname(full_path), exist_ok=True)
        
        # Build template context from research
        research_context = task.metadata.get('research_context', {})
        template_context = {
            "project_name": task.metadata.get("project_name", "QuickOdoo"),
            "api_documentation": research_context.get("documentation_urls", []),
            "api_base_urls": research_context.get("base_urls", []),
            "auth_methods": research_context.get("auth_methods", ["OAuth 2.0"]),
            "detected_entities": research_context.get("entities", []),
            "code_examples": research_context.get("code_examples", [])
        }
        
        # Try to use FULL client template first (supports all 40+ QBO entities)
        if self.template_renderer.template_exists("integration/qbo_client_full.j2"):
            content = self.template_renderer.render("integration/qbo_client_full.j2", template_context)
        elif self.template_renderer.template_exists("integration/qbo_client.j2"):
            content = self.template_renderer.render("integration/qbo_client.j2", template_context)
        else:
            # Fallback inline template
            content = '''"""
...
'''
    ...
    def _create_odoo_integration(self, task: Task, api_info: Dict = None) -> List[str]:
...
'''
    ...
    def _create_stripe_integration(self, task: Task, api_info: Dict = None) -> List[str]:
