...
REM Try to read from .env file using PowerShell (more reliable)
if exist "addon_portal\.env" (
    echo [INFO] Reading database connection from .env file...
    
    REM Use PowerShell to extract password from DB_DSN
    REM Format: postgresql+psycopg://user:password@host:port/database
    for /f "delims=" %%a in ('powershell -NoProfile -Command "$envFile='addon_portal\.env'; if (Test-Path $envFile) { $line = Get-Content $envFile | Where-Object { $_ -match '^DB_DSN=' }; if ($line) { $dsn = $line.Split('=',2)[1]; if ($dsn -match '://([^:]+):([^@]+)@([^:]+):(\d+)/(.+)') { $matches[2] } } }"') do (
        set DB_PASSWORD=%%a
    )
    
    for /f "delims=" %%a in ('powershell -NoProfile -Command "$envFile='addon_portal\.env'; if (Test-Path $envFile) { $line = Get-Content $envFile | Where-Object { $_ -match '^DB_DSN=' }; if ($line) { $dsn = $line.Split('=',2)[1]; if ($dsn -match '://([^:]+):([^@]+)@([^:]+):(\d+)/(.+)') { $matches[1] } } }"') do (
        set DB_USER=%%a
    )
    
    for /f "delims=" %%a in ('powershell -NoProfile -Command "$envFile='addon_portal\.env'; if (Test-Path $envFile) { $line = Get-Content $envFile | Where-Object { $_ -match '^DB_DSN=' }; if ($line) { $dsn = $line.Split('=',2)[1]; if ($dsn -match '://([^:]+):([^@]+)@([^:]+):(\d+)/(.+)') { $matches[3] } } }"') do (
        set DB_HOST=%%a
    )
    
    for /f "delims=" %%a in ('powershell -NoProfile -Command "$envFile='addon_portal\.env'; if (Test-Path $envFile) { $line = Get-Content $envFile | Where-Object { $_ -match '^DB_DSN=' }; if ($line) { $dsn = $line.Split('=',2)[1]; if ($dsn -match '://([^:]+):([^@]+)@([^:]+):(\d+)/(.+)') { $matches[4] } } }"') do (
        set DB_PORT=%%a
    )
    
    for /f "delims=" %%a in ('powershell -NoProfile -Command "$envFile='addon_portal\.env'; if (Test-Path $envFile) { $line = Get-Content $envFile | Where-Object { $_ -match '^DB_DSN=' }; if ($line) { $dsn = $line.Split('=',2)[1]; if ($dsn -match '://([^:]+):([^@]+)@([^:]+):(\d+)/(.+)') { $matches[5] } } }"') do (
        set DB_NAME=%%a
    )
